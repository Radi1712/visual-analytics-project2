 ...
 */
export function kMeans(dataPointsToCluster, k, contributionPerVariable, distanceType = "euclidean") {
    ...
    while (hasChanged) {
        dataObjects = assignDatapointsToCentroids(dataObjects, centroids, contributionPerVariable, distanceType);
    ...
    //return dataObjects;
    return {
        clusters: dataObjects.map(obj => obj.centroidIndex),
        centroids: centroids
    };
}
...
function assignDatapointsToCentroids(
  ...
  distanceType = "euclidean" // default
) {
  return dataObjects.map(dataObject => {
    ...
    centroids.forEach((centroid, index) => {
      const distance =
        distanceType === "manhattan"
          ? distanceManhattanContribution(dataObject.dataPoint, centroid, contributionPerVariable)
          : distanceEuclidianContribution(dataObject.dataPoint, centroid, contributionPerVariable);
    ...
    return { dataPoint: dataObject.dataPoint, centroidIndex };
  });
}
